...
void handle_connect(int socket_fd, packet *received_packet,
...
void handle_terminate(int socket_fd, packet *received_packet,
                                struct sockaddr_in *client_address, socklen_t len){
  client_info *client = find_client(client_address);
...
void handle_data_exchange(int socket_fd, packet *received_packet,
                          struct sockaddr_in *client_address, socklen_t len){
  client_info *client = find_client(client_address);
  ...
  while (1){
    ...
    if (FD_ISSET(server_socket, &read_fds)){
      ...
      if (received_packet.flags & SYN){
        handle_connect(server_socket, &received_packet, &client_address, len);
      }
      
      else if (received_packet.flags & FIN){
        handle_terminate(server_socket, &received_packet, &client_address, len);
      }
      else if (received_packet.flags & ACK){
        handle_acknowledge(&received_packet, &client_address);
      }
      else{
        handle_data_exchange(server_socket, &received_packet, &client_address, len);
      }
    }
  }
}
